...
if ($action == 'update_fullname') {
    ...
    $update = $conn->query("UPDATE files SET fullname = '$fullname' WHERE id = $id");
...
<nav aria-label="breadcrumb ">
    <ol class="breadcrumb">
        ...
        ?> 
        <li class="breadcrumb-item"><a class="text-success" href="indexs.php?page=files">Files</a></li>
    </ol>
</nav>
<div class="container-fluid">
    ...
    <tr>
        <th width="20%" class="">File Number</th>
        <th width="20%" class="">File Name</th>
        <th width="20%" class="">Full Name</th>
    ...
    ?>
        <tr class='file-item' data-id="<?php echo $row['id'] ?>" data-name="<?php echo $name ?>">
            ...
            <td>
                <i class="to_file"><?php echo $row['file_number'] ?></i>
                <input type="text" class="edit-input-file-number" value="<?php echo $row['file_number'] ?>" data-id="<?php echo $row['id'] ?>" style="display: none;">
            </td>
            <td><large><span><i class="fa <?php echo $icon ?>"></i></span><b class="to_file"> <?php echo $name ?></b></large>
            <input type="text" class="rename_file" value="<?php echo $row['name'] ?>" data-id="<?php echo $row['id'] ?>" data-type="<?php echo $row['file_type'] ?>" style="display: none"></td>    
            
            <td><i class="to_file"><?php echo $row['fullname'] ?></i></td>
...
<script>
	...
	$('.folder-item').dblclick(function(){
		location.href = 'indexs.php?page=files&fid='+$(this).attr('data-id')
	})
